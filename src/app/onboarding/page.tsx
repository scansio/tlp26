...
const OPTIONS = {
  ...
  riskPerTrade: ['0.5% – Conservative', '1% – Moderate', '2% – Balanced', '3%+ – Aggressive'],
  ...
  timeframes: ['Scalp (1m–15m)', 'Intraday (1h–4h)', 'Swing (4h–1d)', 'Multi-timeframe'],
} as const;
...
export default function OnboardingPage() {
  ...
  const canProceed = isFinalStep || SELECTION_STEPS[step].fields.every((key) => {
    const v = selections[key];
  ...
  const buildPrompt = () => {
    ...
    const s0 = [
      selections.strategy.length && `Strategy: ${selections.strategy.join(' + ')}`,
      stepNotes[0]?.trim(),
    ].filter(Boolean).join('. ');
    ...
    const s3 = [
      ...
      selections.timeframes.length && `Timeframes: ${selections.timeframes.join(', ')}`,
      stepNotes[3]?.trim(),
    ].filter(Boolean).join('. ');
      ...
      <div className="relative flex flex-1 flex-col p-6">
        ...
        {messages.length === 0 ? (
            ...
            <div className="mx-auto mb-8 flex w-full max-w-md items-center gap-1.5">
              {Array.from({ length: TOTAL_STEPS }).map((_, i) => (
                <motion.div
                  ...
                  className={cn(
                    'h-1 rounded-full',
                    i <= step ? `bg-gradient-to-r ${currentStep.gradient}` : 'bg-muted',
                  )}
              ...
              >
                      ...
                      <div className="space-y-5">
                            ...
                            {SELECTION_STEPS[step].fields.length > 1 && (
                              <p className="mb-2.5 text-xs font-medium text-muted-foreground">
                                ...
                                {MULTI_SELECT.includes(key) && (
                                  <span className="ml-1 opacity-60">· pick multiple</span>
                                )}
                              </p>
                            )}
                            {SELECTION_STEPS[step].fields.length === 1 &&
                        ...
                        <div className="pt-1">
                          <button
                            type="button"
                            onClick={() =>
                              setShowStepNote((prev) => ({ ...prev, [step]: !prev[step] }))
                            }
                            ...
                            {showStepNote[step] && (
                              ...
                              >
                                <textarea
                                  ...
                                  onChange={(e) =>
                                    setStepNotes((prev) => ({ ...prev, [step]: e.target.value }))
                                  }
                ...
                <div className="mt-6 flex items-center gap-3">
                  ...
                  <motion.button
                    ...
                    disabled={!canProceed || (isFinalStep && status !== 'ready')}
                    whileTap={{ scale: 0.97 }}
                    className={cn(
          ...
          <div className="flex flex-1 flex-col overflow-hidden">
            ...
            <PromptInput onSubmit={handleChatSubmit} className="mt-6">
              <PromptInputBody>
                <PromptInputTextarea
                  value={chatInput}
